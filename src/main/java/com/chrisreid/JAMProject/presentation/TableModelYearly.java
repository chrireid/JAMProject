...
public class TableModelYearly extends AbstractTableModel {
	...
	private String[] columnNames = { resources.getString("Sun"),
			resources.getString("Mon"), resources.getString("Tue"),
			resources.getString("Wed"), resources.getString("Thu"),
			resources.getString("Fri"), resources.getString("Sat") };
	...
	public TableModelYearly(Calendar date) {
	...
	private ArrayList<AppointmentBean> fetchAppointments() {
		...
		try {
			 ...
			 log.debug("number of appointments for month (" + date.getTime().toString() + "): " + appointments.size());
			...
			for (int col=0; col<data[row].length; col++) {
				...
				if (data[row][col] != "") {
					...
					for (AppointmentBean ab : appointments) {
						...
						if (ab.getStartTime().before(end) && ab.getEndTime().after(start)) {
							counter++;
						}
					}
					if (counter > 0) {
						...
						daily = String.valueOf(data[row][col]);
					}
				}
